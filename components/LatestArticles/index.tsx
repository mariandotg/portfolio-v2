      ...
      <div className='flex w-full snap-x tablet:col-span-3 snap-mandatory'>
        <ul className='flex flex-col w-full gap-4 mobile:grid mobile:grid-cols-2 mobile:flex-row tablet:gap-8 tablet:grid tablet:grid-cols-3'>
          {data.content.articles.map((article) => (
            ...
            >
              <span className='font-light text-light-text dark:text-dark-text text-secondary'>
                {article.date.start}
              </span>
              ...
              <h3 className='font-medium text-title tablet:text dark:group-hover:text-primary group-hover:text-primary dark:text-dark-headlines font-display text-light-headlines'>
                {article.name}
              </h3>
            </li>
          ))}
